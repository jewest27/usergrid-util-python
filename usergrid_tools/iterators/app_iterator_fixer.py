...
def create_new(app, collection_name, source_entity, attempts=0):
    ...
    try:
        target_entity_url = put_entity_url_template.format(org=config.get('org'),
                                                           app=app,
...
def main():
    ...
    for org_app, app_uuid in r.json().get('data').iteritems():
        ...
        target_app_url = app_url_template.format(org=config.get('org'),
                                                 app=app,
                                                 **config.get('target_endpoint'))
        ...
        if len(app_entities) > 0:
            ...
            for collection_name, collection_data in collections.iteritems():
                ...
                if collection_name not in collections_to_process and '*' not in collections_to_process:
                    logger.warning('Skipping collection=[%s]' % app)
                    continue
